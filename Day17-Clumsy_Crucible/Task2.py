...
while heap:
    ...
    for direction, change in enumerate(directions):
        ...
        if w != -1:
            if (direction == current_direction and current_steps == 10) or is_opposite(direction, current_direction):
                continue
            if current_steps < 4 and direction != current_direction:
                continue
...
path_ends = [heat for ((x, y), _), heat in visited.items() if x == ROW_COUNT - 1 and y == COLUMN_COUNT - 1][1:]
print(min(path_ends))